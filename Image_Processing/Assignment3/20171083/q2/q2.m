...
%PART C
f_temp = padarray(f,[pad_val_1,pad_val_1],'pre');
h_temp = padarray(h,[pad_val_1,pad_val_1],'pre');
f_padded = padarray(f_temp,[pad_val_2,pad_val_2],'post');
h_padded = padarray(h_temp,[pad_val_2,pad_val_2],'post');

F_padded = fft2(f_padded);
H_padded = fft2(h_padded);

ifft_padded_image = ifft2(F_padded.*H_padded);
figure(fig_count);imshow(abs(ifftshift(ifft_padded_image)),[]);title('IFFT Padded IMAGE');fig_count = fig_count + 1;
