...
y = medfilt1(yy);
figure(fig_count); stem(y); title('Apply medfilt'); fig_count = fig_count + 1;

Y = fft(y,Fs);
figure(fig_count); stem(abs(Y)); title('FFT medfilt'); fig_count = fig_count + 1;
...
YY = fft(filtered_sound,Fs);
figure(fig_count); stem(abs(YY)); title('FFT filtered'); fig_count = fig_count + 1;
...
% med = medfilt1(5,'truncate');
% gol = sgolayfilt(med,1,5);
